...
Note: 可利用 ```add_mock_data.py``` 建立假資料測試此功能
...
├── data/
│   ├── assignment.db  # 作業資料庫
│   ├── assignments.json ＃ 作業資料
│   └── cool_state.json ＃COOL 登入狀態
│
└── README.md
```
...
登入 NTU COOL 並建立資料庫，程式會自動建立```data```資料夾及其他檔案

```bash
python main.py
```

(Optional) 建立範例用時資料以協助用時分析頁面顯示
